...
class QwenGuardAdapter(GuardModel):
    """Adapter for Qwen3Guard / Qwen2.5-Instruct safety guard models."""
    ...
    def load(self) -> None:
        if not self.config.get("enabled", False) or not self.checkpoint:
            raise NotImplementedError(
                ...
                f"To run in Colab: set enabled: true and checkpoint: 'Qwen/Qwen2.5-7B-Instruct' (or 'Qwen/Qwen2.5-1.5B-Instruct')."
            )

        logger.info(f"Loading Hugging Face checkpoint: {self.checkpoint}...")
        try:
            import torch
            ...
            logger.info(f"Successfully loaded {self.checkpoint} onto {self.model.device}!")
