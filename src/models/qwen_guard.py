...
logger = setup_logger("QwenGuardAdapter")

class QwenGuardAdapter(GuardModel):
    """Adapter for Qwen open-source safety guard models (e.g., Qwen2.5-1.5B-Instruct / Qwen2.5-7B-Instruct)."""
    ...
    def load(self) -> None:
        if not self.config.get("enabled", False) or not self.checkpoint:
            raise NotImplementedError(
                f"Qwen Guard adapter is disabled or missing checkpoint in config/models.yaml.\n"
                f"Set enabled: true and checkpoint: 'Qwen/Qwen2.5-1.5B-Instruct' (or 'Qwen/Qwen2.5-7B-Instruct')."
            )

        logger.info(f"Loading open-source Hugging Face SLM checkpoint: {self.checkpoint}...")
        try:
            import torch
            ...
            logger.error(f"Failed loading Qwen Guard model '{self.checkpoint}': {e}")
